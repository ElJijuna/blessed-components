...
describe('Blessed Checkbox adapter', () => {
  it('focuses and toggles through Enter, Space, click, and toggle()', () => {
    ...
    try {
      const component = checkbox({
        box: { width: 32 },
        data: { label: 'Include prereleases', onCheckedChange },
        parent: screen,
      });
  ...
  it('keeps controlled state unchanged until new data arrives', () => {
    ...
    try {
      ...
      expect(component.checked()).toBe(false);
      expect(component.element.getContent()).toBe('[ ] Enabled');

      component.setData({ checked: true, label: 'Enabled', onCheckedChange });

      expect(component.checked()).toBe(true);
      expect(component.element.getContent()).toBe('[x] Enabled');
    } finally {
      screen.destroy();
    }
  });
