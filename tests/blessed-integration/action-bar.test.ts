...
describe('Blessed ActionBar adapter', () => {
  it('navigates, activates, updates, focuses, and destroys its element', () => {
    const screen = blessed.screen({ input: new PassThrough(), output: new PassThrough(), terminal: 'xterm' });
    ...
    try {
      const bar = actionBar({
        ...
        data: {
          ...
          capabilities: { unicode: false }, onAction, width: 60,
        }, parent: screen,
      });
      ...
      bar.setData({ actions: [{ id: 'save', label: 'Save' }], capabilities: { unicode: true }, width: 30 });
